...
VALUES (1, 'abbr', 'unparsed_equals', '<abbr title="$1">', '</abbr>', '', '', ' ($1)', '', 0, '', '', '', '', '', '', 'optional', '', 'none'),
	...
	(7, 'code', 'unparsed_content', '', '', '<div class="codeheader">{{code}}: <a href="#" onclick="return smfSelectText(this);" class="codeoperation">{{code_select}}</a></div><code class="bbc_code">$1</code>', '', '', '', 1, '', 'global $context;\r\n\r\nif (!isset($disabled[''code'']))\r\n{\r\n	$php_parts = preg_split(''~(&lt;\\?php|\\?&gt;)~'', $data, -1, PREG_SPLIT_DELIM_CAPTURE);\r\n\r\n	for ($php_i = 0, $php_n = count($php_parts); $php_i < $php_n; $php_i++)\r\n	{\r\n		// Do PHP code coloring?\r\n		if ($php_parts[$php_i] != ''&lt;?php'')\r\n			continue;\r\n\r\n		$php_string = '''';\r\n		while ($php_i + 1 < count($php_parts) && $php_parts[$php_i] != ''?&gt;'')\r\n		{\r\n			$php_string .= $php_parts[$php_i];\r\n			$php_parts[$php_i++] = '''';\r\n		}\r\n		$php_parts[$php_i] = highlight_php_code($php_string . $php_parts[$php_i]);\r\n	}\r\n\r\n	// Fix the PHP code stuff...\r\n	$data = str_replace("<pre style=\\"display: inline;\\">\\t</pre>", "\\t", implode('''', $php_parts));\r\n\r\n	// Older browsers are annoying, aren''t they?\r\n	if (!$context[''browser''][''is_gecko''])\r\n		$data = str_replace("\\t", "<span style=\\"white-space: pre;\\">\\t</span>", $data);\r\n	else\r\n		$data = str_replace("\\t", "&nbsp;&nbsp;&nbsp;", $data);\r\n\r\n	// Recent Opera bug requiring temporary fix. &nsbp; is needed before </code> to avoid broken selection.\r\n	if ($context[''browser''][''is_opera''])\r\n		$data .= ''&nbsp;'';\r\n}', '', '', '', '', 'none', '', 'none'),
	(8, 'code', 'unparsed_equals_content', '', '', '<div class="codeheader">{{code}}: ($2) <a href="#" onclick="return smfSelectText(this);" class="codeoperation"{{code_select}}</a></div><code class="bbc_code">$1</code>', '', '', '', 1, '', 'global $context;\r\n\r\nif (!isset($disabled[''code'']))\r\n{\r\n	$php_parts = preg_split(''~(&lt;\\?php|\\?&gt;)~'', $data[0], -1, PREG_SPLIT_DELIM_CAPTURE);\r\n\r\n	for ($php_i = 0, $php_n = count($php_parts); $php_i < $php_n; $php_i++)\r\n	{\r\n		// Do PHP code coloring?\r\n		if ($php_parts[$php_i] != ''&lt;?php'')\r\n			continue;\r\n\r\n		$php_string = '''';\r\n		while ($php_i + 1 < count($php_parts) && $php_parts[$php_i] != ''?&gt;'')\r\n		{\r\n			$php_string .= $php_parts[$php_i];\r\n			$php_parts[$php_i++] = '''';\r\n		}\r\n		$php_parts[$php_i] = highlight_php_code($php_string . $php_parts[$php_i]);\r\n	}\r\n\r\n	// Fix the PHP code stuff...\r\n	$data[0] = str_replace("<pre style=\\"display: inline;\\">\\t</pre>", "\\t", implode('''', $php_parts));\r\n\r\n	// Older browsers are annoying, aren''t they?\r\n	if (!$context[''browser''][''is_gecko''])\r\n		$data[0] = str_replace("\\t", "<span style=\\"white-space: pre;\\">\\t</span>", $data[0]);\r\n	else\r\n		$data[0] = str_replace("\\t", "&nbsp;&nbsp;&nbsp;", $data[0]);\r\n\r\n	// Recent Opera bug requiring temporary fix. &nsbp; is needed before </code> to avoid broken selection.\r\n	if ($context[''browser''][''is_opera''])\r\n		$data[0] .= ''&nbsp;'';\r\n}', '', '', '', '', 'none', '', 'none'),
	...
	(52, 'url', 'unparsed_content', '', '', '<a href="$1" class="bbc_link new_win" target="_blank">$1</a>', '', '', '', 0, '', '$data = strtr($data, array(''<br />'' => ''''));\r\nif (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', '', '', '', '', 'none', '', 'none'),
	(53, 'url', 'unparsed_equals', '<a href="$1" class="bbc_link new_win" target="_blank">', '</a>', '', '', ' ($1)', '', 0, '', 'if (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', 'email,ftp,url,iurl', '', '', '', 'none', '', 'none');
# --------------------------------------------------------
