...
VALUES (1, 'abbr', 'unparsed_equals', '<abbr title="$1">', '</abbr>', '', '', ' ($1)', '', 0, '', '', '', '', '', '', 'optional', '', 'none'),
	...
	(7, 'code', 'unparsed_content', '', '', '<div class="codeheader">{{code}}: <a href="#" onclick="return smfSelectText(this);" class="codeoperation">{{code_select}}</a></div>', '', '', '', 1, '', 'global $context;\r\n\r\nif (!isset($disabled[''code'']))\r\n{\r\n	if ($context[''browser''][''is_gecko''] || $context[''browser''][''is_opera''])\r\n		$tag[''content''] .= ''<pre style="margin: 0; padding: 0"><code class="bbc_code">$1</code></pre>'';\r\n	else\r\n 		$tag[''content''] .= ''<code class="bbc_code">$1</code>'';\r\n\r\n	$php_parts = preg_split(''~(&lt;\\?php|\\?&gt;)~'', $data, -1, PREG_SPLIT_DELIM_CAPTURE);\r\n\r\n	for ($php_i = 0, $php_n = count($php_parts); $php_i < $php_n; $php_i++)\r\n	{\r\n		// Do PHP code coloring?\r\n		if ($php_parts[$php_i] != ''&lt;?php'')\r\n			continue;\r\n\r\n		$php_string = '''';\r\n		while ($php_i + 1 < count($php_parts) && $php_parts[$php_i] != ''?&gt;'')\r\n		{\r\n			$php_string .= $php_parts[$php_i];\r\n			$php_parts[$php_i++] = '''';\r\n		}\r\n		$php_parts[$php_i] = highlight_php_code($php_string . $php_parts[$php_i]);\r\n	}\r\n\r\n	// Fix the PHP code stuff...\r\n	$data = str_replace("<pre style=\\"display: inline;\\">\\t</pre>", "\\t", implode('''', $php_parts));\r\n\r\n	// Older browsers are annoying, aren''t they?\r\n	if (!$context[''browser''][''is_gecko''])\r\n		$data = str_replace("\\t", "<span style=\\"white-space: pre;\\">\\t</span>", $data);\r\n\r\n	// Recent Opera bug requiring temporary fix. &nbsp; is needed before </code> to avoid broken selection.\r\n	if ($context[''browser''][''is_opera''])\r\n		$data .= ''&nbsp;'';\r\n\r\n	// Fix IE line breaks to actually be copyable.\r\n	if ($context[''browser''][''is_ie''])\r\n		$data = str_replace(''<br>'', ''&#13;'', $data);\r\n}', '', '', '', '', 'none', '', 'none'),
	(8, 'code', 'unparsed_equals_content', '', '', '<div class="codeheader">{{code}}: ($2) <a href="#" onclick="return smfSelectText(this);" class="codeoperation">{{code_select}}</a></div>', '', '', '', 1, '', 'global $context;\r\n\r\nif (!isset($disabled[''code'']))\r\n{\r\n	if ($context[''browser''][''is_gecko''] || $context[''browser''][''is_opera''])\r\n		$tag[''content''] .= ''<pre style="margin: 0; padding: 0"><code class="bbc_code">$1</code></pre>'';\r\n	else\r\n 		$tag[''content''] .= ''<code class="bbc_code">$1</code>'';\r\n\r\n	$php_parts = preg_split(''~(&lt;\\?php|\\?&gt;)~'', $data[0], -1, PREG_SPLIT_DELIM_CAPTURE);\r\n\r\n	for ($php_i = 0, $php_n = count($php_parts); $php_i < $php_n; $php_i++)\r\n	{\r\n		// Do PHP code coloring?\r\n		if ($php_parts[$php_i] != ''&lt;?php'')\r\n			continue;\r\n\r\n		$php_string = '''';\r\n		while ($php_i + 1 < count($php_parts) && $php_parts[$php_i] != ''?&gt;'')\r\n		{\r\n			$php_string .= $php_parts[$php_i];\r\n			$php_parts[$php_i++] = '''';\r\n		}\r\n		$php_parts[$php_i] = highlight_php_code($php_string . $php_parts[$php_i]);\r\n	}\r\n\r\n	// Fix the PHP code stuff...\r\n	$data[0] = str_replace("<pre style=\\"display: inline;\\">\\t</pre>", "\\t", implode('''', $php_parts));\r\n\r\n	// Older browsers are annoying, aren''t they?\r\n	if (!$context[''browser''][''is_gecko''])\r\n		$data[0] = str_replace("\\t", "<span style=\\"white-space: pre;\\">\\t</span>", $data[0]);\r\n\r\n	// Recent Opera bug requiring temporary fix. &nsbp; is needed before </code> to avoid broken selection.\r\n	if ($context[''browser''][''is_opera''])\r\n		$data[0] .= ''&nbsp;'';\r\n\r\n	// Fix IE line breaks to actually be copyable.\r\n	if ($context[''browser''][''is_ie''])\r\n		$data[0] = str_replace(''<br>'', ''&#13;'', $data[0]);\r\n}', '', '', '', '', 'none', '', 'none'),
	...
	(23, 'left', 'parsed', '<div style="text-align: left">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	...
	(29, 'mergedate', 'unparsed_content', '', '', '<div class="mergedate">{{search_date_posted}} $1</div>', '', '', '', 0, '', 'if (is_numeric($data)) $data = timeformat($data);', '', '', '', '', 'none', '', 'none'),
	(30, 'more', 'closed', '', '', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(31, 'nobbc', 'unparsed_content', '', '', '$1', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(32, 'php', 'unparsed_content', '', '', '<code class="php_code">$1</code>', '', '', '$1', 0, '', '$add_begin = substr(trim($data), 0, 5) != ''&lt;'';\r\n$data = highlight_php_code($add_begin ? ''&lt;?php '' . $data . ''?&gt;'' : $data);\r\nif ($add_begin)\r\n	$data = preg_replace(array(''~^(.+?)&lt;\\?.{0,40}?php(?:&nbsp;|\\s)~'', ''~\\?&gt;((?:</(font|span)>)*)$~''), ''$1'', $data, 2);', '', '', '', '', 'none', '', 'none'),
	(33, 'pre', 'parsed', '<pre>', '</pre>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(34, 'quote', 'parsed', '<div class="quoteheader">{{quote}}</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(35, 'quote', 'parsed', '<div class="quoteheader">{{quote_from}} {author}</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', 'a:1:{s:6:"author";a:2:{s:5:"match";s:11:"(.{1,192}?)";s:6:"quoted";b:1;}}', 'none'),
	(36, 'quote', 'parsed_equals', '<div class="quoteheader">{{quote_from}} $1</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', 'url,iurl,ftp', 'optional', '', 'none'),
	(37, 'quote', 'parsed', '<div class="quoteheader"><a href="{{scripturl}}?{link}">{{quote_from}} {author} {{search_on}} {date}</a></div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', 'a:3:{s:6:"author";a:1:{s:5:"match";s:15:"([^<>]{1,192}?)";}s:4:"link";a:1:{s:5:"match";s:83:"(topic=[\\dmsg#\\./]{1,40}(?:;start=[\\dmsg#\\./]{1,40})?|action=profile;u=\\d+|msg=\\d+)";}s:4:"date";a:2:{s:5:"match";s:5:"(\\d+)";s:8:"validate";s:13:"on_timeformat";}}', 'none'),
	(38, 'quote', 'parsed', '<div class="quoteheader">{{quote_from}} {author}</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', 'a:1:{s:6:"author";a:1:{s:5:"match";s:11:"(.{1,192}?)";}}', 'none'),
	(39, 'right', 'parsed', '<div style="text-align: right">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(40, 'rtl', 'parsed', '<div dir="rtl">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(41, 's', 'parsed', '<del>', '</del>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(42, 'size', 'unparsed_equals', '<span style="font-size: $1" class="bbc_size">', '</span>', '', '', '', '', 0, '([1-9][\\d]?p[xt]|small(?:er)?|large[r]?|x[x]?-(?:small|large)|medium|(0\\.[1-9]|[1-9](\\.[\\d][\\d]?)?)?em)\\]', '', '', '', '', '', 'none', '', 'none'),
	(43, 'size', 'unparsed_equals', '<span style="font-size: $1" class="bbc_size">', '</span>', '', '', '', '', 0, '[1-7]\\]', '$sizes = array(1 => 0.7, 2 => 1.0, 3 => 1.35, 4 => 1.45, 5 => 2.0, 6 => 2.65, 7 => 3.95);\r\n$data = $sizes[$data] . ''em'';', '', '', '', '', 'none', '', 'none'),
	(44, 'spoiler', 'parsed', '<div class="spoiler"><div class="spoilerhint"><input type="button" value="{{spoiler}}" onclick="n = this.parentNode.parentNode.lastChild.style; n.display = n.display == ''none'' ? ''block'' : ''none''; return false;">{{click_for_spoiler}}</div><div class="spoiled" style="display: none">', '</div></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(45, 'sub', 'parsed', '<sub>', '</sub>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(46, 'sup', 'parsed', '<sup>', '</sup>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(47, 'table', 'parsed', '<table class="bbc_table">', '</table>', '', '', '', '', 1, '', '', '', '', 'tr', '', 'none', '', 'inside'),
	(48, 'td', 'parsed', '<td>', '</td>', '', ' ', ' ', '', 1, '', '', '', 'tr', '', '', 'none', '', 'outside'),
	(49, 'time', 'unparsed_content', '', '', '$1', '', '', '', 0, '', 'if (is_numeric($data))\r\n	$data = timeformat($data);\r\nelse\r\n	$tag[''content''] = ''[time]$1[/time]'';', '', '', '', '', 'none', '', 'none'),
	(50, 'tr', 'parsed', '<tr>', '</tr>', '', ' ', ' ', '', 1, '', '', '', 'table', 'td', '', 'none', '', 'both'),
	(51, 'tt', 'parsed', '<span class="bbc_tt">', '</span>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(52, 'u', 'parsed', '<span class="bbc_u">', '</span>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(53, 'url', 'unparsed_content', '', '', '<a href="$1" class="bbc_link" target="_blank">$1</a>', '', '', '', 0, '', '$data = strtr($data, array(''<br>'' => ''''));\r\nif (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', '', '', '', '', 'none', '', 'none'),
	(54, 'url', 'unparsed_equals', '<a href="$1" class="bbc_link" target="_blank">', '</a>', '', '', ' ($1)', '', 0, '', 'if (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', 'email,ftp,url,iurl', '', '', '', 'none', '', 'none');
# --------------------------------------------------------
