...
#### Install script for MySQL 4.1.2+
...
VALUES (1, 'abbr', 'unparsed_equals', '<abbr title="$1">', '</abbr>', '', '', ' ($1)', '', 0, '', '', '', '', '', '', 'optional', '', 'none'),
	...
	(5, 'black', 'parsed', '<span style="color: black" class="bbc_color">', '</span>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(6, 'blue', 'parsed', '<span style="color: blue" class="bbc_color">', '</span>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(7, 'br', 'closed', '', '', '<br />', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(8, 'center', 'parsed', '<div class="centertext">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(9, 'code', 'unparsed_content', '', '', '<div class="codeheader">{{code}}: <a href="#" onclick="return smfSelectText(this);" class="codeoperation">{{code_select}}</a></div><code class="bbc_code">$1</code>', '', '', '', 1, '', 'global $context;\r\n\r\nif (!isset($disabled[''code'']))\r\n{\r\n	$php_parts = preg_split(''~(&lt;\\?php|\\?&gt;)~'', $data, -1, PREG_SPLIT_DELIM_CAPTURE);\r\n\r\n	for ($php_i = 0, $php_n = count($php_parts); $php_i < $php_n; $php_i++)\r\n	{\r\n		// Do PHP code coloring?\r\n		if ($php_parts[$php_i] != ''&lt;?php'')\r\n			continue;\r\n\r\n		$php_string = '''';\r\n		while ($php_i + 1 < count($php_parts) && $php_parts[$php_i] != ''?&gt;'')\r\n		{\r\n			$php_string .= $php_parts[$php_i];\r\n			$php_parts[$php_i++] = '''';\r\n		}\r\n		$php_parts[$php_i] = highlight_php_code($php_string . $php_parts[$php_i]);\r\n	}\r\n\r\n	// Fix the PHP code stuff...\r\n	$data = str_replace("<pre style=\\"display: inline;\\">\\t</pre>", "\\t", implode('''', $php_parts));\r\n\r\n	// Older browsers are annoying, aren''t they?\r\n	if (!$context[''browser''][''is_gecko''])\r\n		$data = str_replace("\\t", "<span style=\\"white-space: pre;\\">\\t</span>", $data);\r\n	else\r\n		$data = str_replace("\\t", "&nbsp;&nbsp;&nbsp;", $data);\r\n\r\n	// Recent Opera bug requiring temporary fix. &nsbp; is needed before </code> to avoid broken selection.\r\n	if ($context[''browser''][''is_opera''])\r\n		$data .= ''&nbsp;'';\r\n}', '', '', '', '', 'none', '', 'none'),
	(10, 'code', 'unparsed_equals_content', '', '', '<div class="codeheader">{{code}}: ($2) <a href="#" onclick="return smfSelectText(this);" class="codeoperation"{{code_select}}</a></div><code class="bbc_code">$1</code>', '', '', '', 1, '', 'global $context;\r\n\r\nif (!isset($disabled[''code'']))\r\n{\r\n	$php_parts = preg_split(''~(&lt;\\?php|\\?&gt;)~'', $data[0], -1, PREG_SPLIT_DELIM_CAPTURE);\r\n\r\n	for ($php_i = 0, $php_n = count($php_parts); $php_i < $php_n; $php_i++)\r\n	{\r\n		// Do PHP code coloring?\r\n		if ($php_parts[$php_i] != ''&lt;?php'')\r\n			continue;\r\n\r\n		$php_string = '''';\r\n		while ($php_i + 1 < count($php_parts) && $php_parts[$php_i] != ''?&gt;'')\r\n		{\r\n			$php_string .= $php_parts[$php_i];\r\n			$php_parts[$php_i++] = '''';\r\n		}\r\n		$php_parts[$php_i] = highlight_php_code($php_string . $php_parts[$php_i]);\r\n	}\r\n\r\n	// Fix the PHP code stuff...\r\n	$data[0] = str_replace("<pre style=\\"display: inline;\\">\\t</pre>", "\\t", implode('''', $php_parts));\r\n\r\n	// Older browsers are annoying, aren''t they?\r\n	if (!$context[''browser''][''is_gecko''])\r\n		$data[0] = str_replace("\\t", "<span style=\\"white-space: pre;\\">\\t</span>", $data[0]);\r\n	else\r\n		$data[0] = str_replace("\\t", "&nbsp;&nbsp;&nbsp;", $data[0]);\r\n\r\n	// Recent Opera bug requiring temporary fix. &nsbp; is needed before </code> to avoid broken selection.\r\n	if ($context[''browser''][''is_opera''])\r\n		$data[0] .= ''&nbsp;'';\r\n}', '', '', '', '', 'none', '', 'none'),
	(11, 'color', 'unparsed_equals', '<span style="color: $1" class="bbc_color">', '</span>', '', '', '', '', 0, '(#[\\da-fA-F]{3}|#[\\da-fA-F]{6}|[A-Za-z]{1,20}|rgb\\(\\d{1,3}, ?\\d{1,3}, ?\\d{1,3}\\))\\]', '', '', '', '', '', 'none', '', 'none'),
	(12, 'email', 'unparsed_content', '', '', '<a href="mailto:$1" class="bbc_email">$1</a>', '', '', '', 0, '', '$data = strtr($data, array(''<br />'' => ''''));', '', '', '', '', 'none', '', 'none'),
	(13, 'email', 'unparsed_equals', '<a href="mailto:$1" class="bbc_email">', '</a>', '', '', ' ($1)', '', 0, '', '', 'email,ftp,url,iurl', '', '', '', 'none', '', 'none'),
	(14, 'flash', 'unparsed_commas_content', '', '', '<object width="$2" height="$3" data="$1"><param name="movie" value="$1" /><param name="play" value="true" /><param name="loop" value="true" /><param name="quality" value="high" /><param name="allowscriptaccess" value="never" /><embed src="$1" type="application/x-shockwave-flash" allowscriptaccess="never" width="$2" height="$3" /></object>', '', '', '<a href="$1" target="_blank" class="new_win">$1</a>', 0, '\\d+,\\d+\\]', 'if (isset($disabled[''url'']))\r\n	$tag[''content''] = ''$1'';\r\nelseif (strpos($data[0], ''http://'') !== 0 && strpos($data[0], ''https://'') !== 0)\r\n	$data[0] = ''http://'' . $data[0];', '', '', '', '', 'none', '', 'none'),
	(15, 'font', 'unparsed_equals', '<span style="font-family: $1" class="bbc_font">', '</span>', '', '', '', '', 0, '[A-Za-z0-9_,\\-\\s]+?\\]', '', '', '', '', '', 'none', '', 'none'),
	(16, 'ftp', 'unparsed_content', '', '', '<a href="$1" class="bbc_ftp new_win" target="_blank">$1</a>', '', '', '', 0, '', '$data = strtr($data, array(''<br />'' => ''''));\r\nif (strpos($data, ''ftp://'') !== 0 && strpos($data, ''ftps://'') !== 0)\r\n	$data = ''ftp://'' . $data;', '', '', '', '', 'none', '', 'none'),
	(17, 'ftp', 'unparsed_equals', '<a href="$1" class="bbc_ftp new_win" target="_blank">', '</a>', '', '', ' ($1)', '', 0, '', 'if (strpos($data, ''ftp://'') !== 0 && strpos($data, ''ftps://'') !== 0)\r\n	$data = ''ftp://'' . $data;', 'email,ftp,url,iurl', '', '', '', 'none', '', 'none'),
	(18, 'green', 'parsed', '<span style="color: green" class="bbc_color">', '</span>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(19, 'html', 'unparsed_content', '', '', '$1', '', '', '$1', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(20, 'hr', 'closed', '', '', '<hr />', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(21, 'i', 'parsed', '<em>', '</em>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(22, 'img', 'unparsed_content', '', '', '<img src="$1" alt="{alt}"{width}{height} class="bbc_img resized" />', '', '', '($1)', 0, '', '$data = strtr($data, array(''<br />'' => ''''));\r\nif (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', '', '', '', '', 'none', 'a:3:{s:3:"alt";a:1:{s:8:"optional";b:1;}s:5:"width";a:3:{s:8:"optional";b:1;s:5:"value";s:11:" width="$1"";s:5:"match";s:5:"(\\d+)";}s:6:"height";a:3:{s:8:"optional";b:1;s:5:"value";s:12:" height="$1"";s:5:"match";s:5:"(\\d+)";}}', 'none'),
	(23, 'img', 'unparsed_content', '', '', '<img src="$1" class="bbc_img" />', '', '', '($1)', 0, '', '$data = strtr($data, array(''<br />'' => ''''));\r\nif (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', '', '', '', '', 'none', '', 'none'),
	(24, 'iurl', 'unparsed_content', '', '', '<a href="$1" class="bbc_link">$1</a>', '', '', '', 0, '', '$data = strtr($data, array(''<br />'' => ''''));\r\nif (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', '', '', '', '', 'none', '', 'none'),
	(25, 'iurl', 'unparsed_equals', '<a href="$1" class="bbc_link">', '</a>', '', '', ' ($1)', '', 0, '', 'if (substr($data, 0, 1) == ''#'')\r\n	$data = ''#post_'' . substr($data, 1);\r\nelseif (strpos($data, ''http://'') !== 0 && strpos($data, ''https://'') !== 0)\r\n	$data = ''http://'' . $data;', 'email,ftp,url,iurl', '', '', '', 'none', '', 'none'),
	(26, 'left', 'parsed', '<div style="text-align: left">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(27, 'li', 'parsed', '<li>', '</li>', '', ' ', '<br />', '', 1, '', '', '', 'list', '', '', 'none', '', 'outside'),
	(28, 'list', 'parsed', '<ul class="bbc_list">', '</ul>', '', '', '', '', 1, '', '', '', '', 'li,list', '', 'none', '', 'inside'),
	(29, 'list', 'parsed', '<ul class="bbc_list" style="list-style-type: {type}">', '</ul>', '', '', '', '', 1, '', '', '', '', 'li,list', '', 'none', 'a:1:{s:4:"type";a:1:{s:5:"match";s:227:"(none|disc|circle|square|decimal|decimal-leading-zero|lower-roman|upper-roman|lower-alpha|upper-alpha|lower-greek|lower-latin|upper-latin|hebrew|armenian|georgian|cjk-ideographic|hiragana|katakana|hiragana-iroha|katakana-iroha)";}}', 'inside'),
	(30, 'ltr', 'parsed', '<div dir="ltr">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(31, 'me', 'unparsed_equals', '<div class="meaction">* $1 ', '</div>', '', '/me ', '<br />', '', 1, '', '', '', '', '', '', 'optional', '', 'none'),
	(32, 'mergedate', 'unparsed_content', '', '', '<div class="mergedate">{{search_date_posted}} $1</div>', '', '', '', 0, '', 'if (is_numeric($data)) $data = timeformat($data);', '', '', '', '', 'none', '', 'none'),
	(33, 'nobbc', 'unparsed_content', '', '', '$1', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(34, 'php', 'unparsed_content', '', '', '<code class="php_code">$1</code>', '', '', '$1', 0, '', '$add_begin = substr(trim($data), 0, 5) != ''&lt;'';\r\n$data = highlight_php_code($add_begin ? ''&lt;?php '' . $data . ''?&gt;'' : $data);\r\nif ($add_begin)\r\n	$data = preg_replace(array(''~^(.+?)&lt;\\?.{0,40}?php(?:&nbsp;|\\s)~'', ''~\\?&gt;((?:</(font|span)>)*)$~''), ''$1'', $data, 2);', '', '', '', '', 'none', '', 'none'),
	(35, 'pre', 'parsed', '<pre>', '</pre>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(36, 'quote', 'parsed', '<div class="quoteheader">{{quote}}</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(37, 'quote', 'parsed', '<div class="quoteheader">{{quote_from}} {author}</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', 'a:1:{s:6:"author";a:2:{s:5:"match";s:11:"(.{1,192}?)";s:6:"quoted";b:1;}}', 'none'),
	(38, 'quote', 'parsed_equals', '<div class="quoteheader">{{quote_from}} $1</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', 'url,iurl,ftp', 'optional', '', 'none'),
	(39, 'quote', 'parsed', '<div class="quoteheader"><a href="{{scripturl}}?{link}">{{quote_from}} {author} {{search_on}} {date}</a></div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', 'a:3:{s:6:"author";a:1:{s:5:"match";s:15:"([^<>]{1,192}?)";}s:4:"link";a:1:{s:5:"match";s:83:"(topic=[\\dmsg#\\./]{1,40}(?:;start=[\\dmsg#\\./]{1,40})?|action=profile;u=\\d+|msg=\\d+)";}s:4:"date";a:2:{s:5:"match";s:5:"(\\d+)";s:8:"validate";s:13:"on_timeformat";}}', 'none'),
	(40, 'quote', 'parsed', '<div class="quoteheader">{{quote_from}} {author}</div><blockquote>', '</blockquote><div class="quotefooter"></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', 'a:1:{s:6:"author";a:1:{s:5:"match";s:11:"(.{1,192}?)";}}', 'none'),
	(41, 'red', 'parsed', '<span style="color: red" class="bbc_color">', '</span>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(42, 'right', 'parsed', '<div style="text-align: right">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(43, 'rtl', 'parsed', '<div dir="rtl">', '</div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(44, 's', 'parsed', '<del>', '</del>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(45, 'size', 'unparsed_equals', '<span style="font-size: $1" class="bbc_size">', '</span>', '', '', '', '', 0, '([1-9][\\d]?p[xt]|small(?:er)?|large[r]?|x[x]?-(?:small|large)|medium|(0\\.[1-9]|[1-9](\\.[\\d][\\d]?)?)?em)\\]', '', '', '', '', '', 'none', '', 'none'),
	(46, 'size', 'unparsed_equals', '<span style="font-size: $1" class="bbc_size">', '</span>', '', '', '', '', 0, '[1-7]\\]', '$sizes = array(1 => 0.7, 2 => 1.0, 3 => 1.35, 4 => 1.45, 5 => 2.0, 6 => 2.65, 7 => 3.95);\r\n$data = $sizes[$data] . ''em'';', '', '', '', '', 'none', '', 'none'),
	(47, 'spoiler', 'parsed', '<div class="spoiler"><div class="spoilerhint"><input type="button" value="{{spoiler}}" onclick="n = this.parentNode.parentNode.lastChild.style; n.display = n.display == ''none'' ? ''block'' : ''none''; return false;" />{{click_for_spoiler}}</div><div class="spoiled" style="display: none">', '</div></div>', '', '', '', '', 1, '', '', '', '', '', '', 'none', '', 'none'),
	(48, 'sub', 'parsed', '<sub>', '</sub>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(49, 'sup', 'parsed', '<sup>', '</sup>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none'),
	(50, 'table', 'parsed', '<table class="bbc_table">', '</table>', '', '', '', '', 1, '', '', '', '', 'tr', '', 'none', '', 'inside'),
	...
	(58, 'white', 'parsed', '<span style="color: white" class="bbc_color">', '</span>', '', '', '', '', 0, '', '', '', '', '', '', 'none', '', 'none');
# --------------------------------------------------------
